...
本文记录维护者本地隔离验收环境。下文引用的 `测试环境/`、`报告/`、包装脚本、密钥及原始日志均不随公共仓库发布；克隆仓库后不能直接执行这些环境管理命令。通用验收要求见 [验收说明](验收说明.md)。

## 位置与隔离范围

- 所有工具、下载、配置、日志、临时文件、SSH 专用密钥和虚拟磁盘位于 项目根目录下的 `测试环境/`。
...
- 初次验证前后，宿主机 `~/.lima` 和 `~/Library/Caches/lima` 均不存在。
...
2026-09-03 16:08:05（UTC+8）最终查询：`d12`、`u24` 均为 `Stopped`，不再占用虚拟机运行内存。Ubuntu 停止前没有任何 loop 设备，也没有专用样本遗留挂载；所有八份有效隔离报告的清理错误均为空。宿主 `~/.lima` 与 `~/Library/Caches/lima` 仍不存在。
...
以下命令在 项目根目录下的 `测试环境/` 执行，必须通过本目录包装脚本调用，避免误用默认数据目录。
...
- 初次基础检查包括：2 核、免密 sudo、SSH 服务活动、没有挂载宿主用户目录、没有 9p/virtiofs/sshfs 主机共享挂载、没有已有 Docker socket。此后仅为本项目测试新装 Docker。
