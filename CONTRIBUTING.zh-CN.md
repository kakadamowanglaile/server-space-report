...
源码包同时包含 `工具/核对交付包.py`，用于核对发布摘要、源码内容、运行代码与已验收候选的一致性，以及解压后的测试结果。执行 `python3 -B 工具/核对交付包.py --help` 查看参数。`--release` 指定待核对的发布目录，`--candidate` 指定已验收候选目录，`--test-count` 必须填写该候选验收记录中的预期测试总数。不要沿用历史数量，也不要仅为让失败的核对通过而调整数量；测试集变化后应重新验证，再使用新的数量。核对工具须从构建该发布包时使用的源码树执行，源码发生后续修改时应在新目录重新构建。

本地 `发布清单.json` 必须分别列出该版本中文名称的 `.pyz` 运行包和 `-源码.zip` 源码包，各出现一次，顺序不限，并附各自的大小和 SHA256 摘要。重复记录或其他文件名会在读取交付文件前被拒绝。核对工具接受本地构建脚本的命名方式；公开 v0.1.2 附件使用英文名称及 `release-manifest.json`，不能直接交给该本地工具核对。公开附件的大小和 SHA256 应与其下载清单比对。
