...
The local `发布清单.json` must list the version's Chinese-named `.pyz` runtime and `-源码.zip` source archive exactly once each, in either order, with their own sizes and SHA256 hashes. Duplicate entries or other filenames are rejected before the artifacts are read. The checker accepts the local builder's naming scheme. Published v0.1.2 attachments use English names and `release-manifest.json`; they cannot be passed directly to this local checker. Use the published manifest to verify their sizes and SHA256 hashes.
