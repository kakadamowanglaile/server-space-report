...
The release manifest must list the version's `.pyz` runtime and `-源码.zip` source archive exactly once each, in either order, with their own sizes and SHA256 hashes. Duplicate entries or other filenames are rejected before the artifacts are read.
